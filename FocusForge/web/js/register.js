...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Restore saved language immediately (before DOMContentLoaded)
    const savedLang = localStorage.getItem("language") || "en";
    document.getElementById("auth-lang-select").value = savedLang;
    changeAuthLanguage(savedLang);

    // ── All button logic inside one DOMContentLoaded ───────
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Terms controls register button only
        registerBtn.disabled = !cb.checked;
        ...
        registerBtn.addEventListener("click", async (e) => {
            ...
            if (!username || !password) {
                Swal.fire({ title:"Error!", text:"Enter username and password!", icon:"error" });
                return;
            }
            if (password !== confirm_password) {
            ...
            try {
                ...
                if (response.ok) {
                    Swal.fire({ title:"Success", text:"You registered successfully!", icon:"success" });
                    window.location.href = "/login";
                } else {
                    const text = await response.text();
        ...
        // Google — NO terms check needed for OAuth
        if (googleBtn) {
            googleBtn.addEventListener("click", async () => {
                // Warn if terms not checked, but still allow after confirmation
                if (!cb.checked) {
                ...
                try {
                    ...
                    const { initializeApp, getApps } = await import("https://www.gstatic.com/firebasejs/12.12.0/firebase-app.js");
                    const { getAuth, GoogleAuthProvider, signInWithPopup } = await import("https://www.gstatic.com/firebasejs/12.12.0/firebase-auth.js");
                    ...
                    const provider = new GoogleAuthProvider();
                    auth.languageCode = savedLang === "gr" ? "el" : savedLang;
